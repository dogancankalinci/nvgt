...
On Android your assets are packed into the APK rather than written to a folder on the device, but this is invisible to your script: a relative path such as `sounds/footstep.ogg` is looked up in the APK's assets when it is not found on the filesystem. This means that `sound.load`, `pack` and the datastreams open assets directly, and that the filesystem functions which only read, namely `file_exists`, `directory_exists`, `file_get_size`, `find_files`, `find_directories` and `glob`, search the assets as well as the filesystem and report the two together. The one thing that genuinely differs is that assets are read only, so anything your game needs to write must go somewhere else, such as the directory returned by `get_preferences_path`. Note also that only relative paths can name an asset; an absolute path always refers to the real filesystem.
...
* iOS (.app/.ipa): NVGT builds a complete, App Store valid icon set without a Mac or Xcode. It writes the exact size loose PNGs Apple's validator requires in the bundle root (120x120 for iPhone and 152x152 for iPad) together with the matching Info.plist icon entries, and it also generates a real compiled asset catalog (Assets.car) that contains your image as the app icon. Both parts are needed for a store submission, and NVGT produces them for you. Two caveats: the source must be a genuine PNG, so an image that is actually a JPEG renamed to .png will fail to load; and if you omit the pragma altogether, an iOS build ends up with no icon at all rather than a placeholder, which the App Store rejects before your app ever reaches review.
