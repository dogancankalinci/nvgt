...
Install OpenSSL for Windows if you do not already have it. Note that the OpenSSL project itself does not, as of this writing, publish an official first-party Windows build; it ships source code and points to third-party binary distributions. The easiest reputable option is **Shining Light Productions'** build, which you can install in one command with Windows' built-in package manager:

```
winget install -e --id ShiningLight.OpenSSL.Light
```

The **Light** package is all you need, since it contains the `openssl.exe` command line tool. (There is also a much larger `ShiningLight.OpenSSL.Dev` developer edition, around 825 MB with headers and libraries, that you do not need just to build a `.p12`.) If you prefer a manual download instead of winget, get it from [slproweb.com](https://slproweb.com/products/Win32OpenSSL.html). One thing that trips people up: in Windows this installer labels its publisher **"OpenSSL Win64 Installer Team"** (the name shown in Windows' installed-programs list and in the setup file's details). It sounds official, but that is just the name Shining Light Productions gives its own Windows packaging of OpenSSL, not the official OpenSSL organization. The `openssl.exe` you end up with is compiled from the genuine OpenSSL project source (its own file details even read "The OpenSSL Project"); only the Windows installer is the third party's. Note also that these installers are not digitally signed, so Windows shows an "unknown publisher" prompt when you run the setup, which is expected for this distribution. Alternatively, if you already have Git for Windows installed, it bundles an `openssl.exe` you can use.

Then work in an empty folder and run the commands below.
...
#### Watching it happen: the Build Uploads table
Open your app in App Store Connect, go to the **TestFlight** tab, and look at the top of the **iOS Builds** page. Above the list of versions there is a **Build Uploads** table, listing each upload with a status of *Processing*, *Complete*, or *Failed*, a creation date, and — this is the useful part — the warnings, errors and upload log behind that status. A *Failed* row tells you what went wrong without waiting for an email, and Apple lets you reuse the same build number after a failed upload.

Do not sit watching it during the transfer, though: **the row does not appear while the bytes are still going up.** It appears once the delivery is committed and processing begins, and the date it shows is that moment rather than the moment you started. On a slow line that is a long silence — an upload that ran from 15:11 to 15:53 produced a row stamped 15:53 and nothing before it.

There is a catch that will confuse you if you have shipped this app before: **that table only shows builds delivered through Method B or Method C.** Under Method A the row never appears. This is not a display quirk, it is a difference in what the two paths create on Apple's side. Method B and Method C register a build record *before a single byte of the `.ipa` is sent* — that is what the `POST builds` call at the start of the delivery does — and then attach the description and the binary to it. That record genuinely exists for the whole upload: query Apple's delivery API for your build number while the transfer is running and it answers with the build and a null processing state. App Store Connect simply does not render it until the delivery is committed, which is why the table stays empty until the end. Method A hands the `.itmsp` package to the older ingestion pipeline, which creates no such record at any point; there is nothing for the API to return and nothing for the table to show, before or after.

This also settles something that puzzles people who have only ever shipped through Method A. Every tutorial and video about uploading from Xcode says the build will sit in App Store Connect as *Processing* for a while before it is ready, and that has been true for years — but under Method A you never see it, because there is no record to put in that state. The build simply materialises when processing ends, or does not. The visibility is not new; your uploads were just not on the path that has it. Xcode has always delivered through the same modern API that Methods B and C use, which is why its uploads have always been visible and `.itmsp` uploads have not. (The Build Uploads table itself, with its per-upload logs and warnings, is a more recent addition to App Store Connect layered on top of that same data.)

Querying Apple's delivery API for an app whose recent builds were delivered both ways makes the split obvious: the builds uploaded with `-f` have no delivery record at all, while the one uploaded with `-assetFile` is there with its state. Migrating to Method B is what puts your uploads on the visible path, and it is a genuine reason to prefer it beyond simply being the method Apple still accepts — Transporter can and does report a transfer as failed when Apple in fact assembled and accepted it, and this table is where you check which of the two actually happened.

#### A silent failure to know about: "The software asset has already been uploaded."
There is one way an upload can *claim* success while delivering nothing, and it is worth knowing in advance because the log line that gives it away is a warning, not an error. The run finishes in seconds instead of minutes, prints `WARN: The software asset has already been uploaded.` followed by a delivery record, ends with "1 package was uploaded successfully", exits zero — and your build never appears, because no bytes were sent.

What happened: before uploading, Transporter checks whether this build has already been delivered — a reasonable idempotency guard — but it looks the build record up **by build number alone**, ignoring the marketing version. Apple's documented rule for iOS is that a build is identified by the *pair*: App Store Connect requires each app version to use a unique combination of `CFBundleShortVersionString` and `CFBundleVersion`, which is precisely why resetting the build number to 1 for each new marketing version is legal, and why 1.0 (1) and 1.1 (1) are different builds. The server upholds that rule — hand it the full pair and it opens a distinct build record without complaint. The client's pre-upload check does not: if a *different* marketing version's build record with the *same number* is still lying around, the check matches it, sees its asset is complete, and skips your upload.

The saving grace, and the reason this stays rare, is that those server-side delivery records are not kept forever — they age out after a couple of weeks and appear to be pruned sooner as later builds in their own train supersede them. So a team that ships many builds per version, or that never reuses numbers, can reset to 1 for years without ever colliding. The window opens on one specific shape: a short-lived version (say, a single build 1) followed within a week or two by the next version reusing build 1. Then the stale record is still resident, the version-blind lookup finds it, and the new upload is silently absorbed.

Three things follow. First, do not treat the success line as proof of delivery for a first upload of a version — the build appearing in App Store Connect is the proof, and after this warning it will not appear. Second, this is not a Windows quirk, and not even a Transporter quirk: the same engine sits inside Xcode's ContentDeliveryServices framework, `altool`, and the Transporter app, and Mac pipelines have reported exactly this symptom since 2022 ([fastlane #20811](https://github.com/fastlane/fastlane/issues/20811), still open); the workaround that circulated back then — forcing the legacy `.itmsp` path — died when Apple retired that path, so the bug is now unavoidable when the window is open. Method C's `ios-uploader` is caught too, because its pre-upload check copies the same version-only lookup — handed a new version's build 1 while an older version's build 1 is still resident, it reports that build's state as if it were yours and refuses with "A build with version 1 is already uploaded". Credit where due: refusing loudly is the safer failure, since nothing pretends to have delivered — but it blocks the upload all the same. Its build *registration*, unlike its check, does send the full version pair, which is exactly how a corrected client gets through. As of September 2026 nobody has reported this on its issue tracker — the nearest miss is a 2020 report of the *legacy* flow's redundant uploads, which was fixed by relying on the old server-side check that compared both numbers correctly — so do not expect the version-blind lookup fixed upstream until someone files it. Third, resetting build numbers per version remains your documented right on iOS (macOS is the opposite: there the build number must keep increasing across versions), so the durable answers are to verify after upload, and if you do hit the collision, either wait the stale record out or deliver through a client that registers the build with the full version-pair before uploading — the server, unlike the client, gets it right.

And there is a fourth answer, found by dissecting the two installers: **Apple has already fixed this in Transporter 4.2 — silently.** The 4.2 build removes the class that carried the version-blind check and its warning text entirely, and replaces the whole flow with one built on the official Build Upload API, whose requests carry both halves of the identity; the "already uploaded" decision now lives in a checksum utility and its message reads "The package has already been uploaded **(checksum match)**. Skipping upload." — a skip that can only fire when the file really is the same bytes, which is the correct rule. No release note mentions any of this, which fits the pattern: the sibling duplicate-detection bugs Apple fixed in the Transporter *app* in 2020-21 did get release-note lines, but the CLI's fix shipped unannounced. So the wording of the warning tells you which engine you are on — "The software asset has already been uploaded." is the buggy 4.1 check, "(checksum match)" is the fixed 4.2 one — and upgrading to 4.2 is the real remedy for this trap.
...
3. In NVGT, set `build.product_version` to that new number (for example `1.0.1`). You may reset `build.product_version_code` back to `1` for the new version, since on iOS build numbers only need to be unique within a marketing version — but read the note at the end of 9g first: a client-side Transporter bug can silently swallow an upload that reuses a recent short-lived version's build number, so after any such upload confirm the build actually appears.
