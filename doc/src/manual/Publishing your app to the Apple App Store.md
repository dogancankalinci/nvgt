...
The **Light** package is all you need, since it contains the `openssl.exe` command line tool. (There is also a much larger `ShiningLight.OpenSSL.Dev` developer edition, around 825 MB with headers and libraries, that you do not need just to build a `.p12`.) If you prefer a manual download instead of winget, get it from [slproweb.com](https://slproweb.com/products/Win32OpenSSL.html). One thing that trips people up: in Windows this installer shows its **Publisher as "OpenSSL Win64 Installer Team"**, which sounds official but is simply Shining Light's own code-signing label; it is a trusted third party, not the official OpenSSL organization. Alternatively, if you already have Git for Windows installed, it bundles an `openssl.exe` you can use.
